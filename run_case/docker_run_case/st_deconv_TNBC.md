# Reproducing Results: ST Deconvolution on TNBC (CID44971)

This document describes how to reproduce the **TNBC spatial transcriptomics (ST) deconvolution** results (sample **CID44971**) using our Docker image.

---

## 1) Download the `ST_TNBC` package

Download the folder **`ST_TNBC/`** from **`XXXX`**.
...
- `INPUT_DIR`: where you placed `ST_TNBC/input`
...
docker run --rm -it \
  ...
  st_deconv \
  --sc /inputs/SC_CID44971.h5ad \
  --st /inputs/ST_CID44971.h5ad \
  --annotation_key celltype_minor \
  --out_dir "/outputs/" \
  --dataset_name CID44971 \
  --n_cell 8 \
  --seed 42 \
  --reproduce True
```
...
- `-v "${INPUT_DIR}":/inputs:ro`  
  ...
  **Container side (`/inputs`):**
  - The pipeline will read:
    - `/inputs/SC_CID44971.h5ad`
    - `/inputs/ST_CID44971.h5ad`
...
- `--out_dir <path>`  
  ...
  **Important for reproduction:**  
  ...
  - `/outputs/.../st_model/` (copied from `ST_TNBC/st_model/`)
