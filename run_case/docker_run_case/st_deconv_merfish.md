# Reproducing Results: ST Deconvolution on MERFISH (merfish)

This document describes how to reproduce the **MERFISH spatial transcriptomics (ST) deconvolution** results using our Docker image.

---

## 1) Download the `ST_merfish` package

Download the folder **`ST_merfish/`** from **`XXXX`**.
...
- `INPUT_DIR`: where you placed `ST_merfish/input`
...
docker run --rm -it \
  ...
  st_deconv \
  --sc /inputs/merfish_sc_adata.h5ad \
  --st /inputs/merfish_spots_data.h5ad \
  --annotation_key cell_class \
  --out_dir "/outputs/" \
...
- `-v "${INPUT_DIR}":/inputs:ro`  
  ...
  **Container side (`/inputs`):**
  - The pipeline will read:
    - `/inputs/merfish_sc_adata.h5ad`
    - `/inputs/merfish_spots_data.h5ad`
...
- `--out_dir <path>`  
  ...
  **Important for reproduction:**  
  ...
  - `/outputs/.../st_model/` (copied from `ST_merfish/st_model/`)
