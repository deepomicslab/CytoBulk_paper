# Reproducing Results: ST Deconvolution on ER2 (CID4535)

This document describes how to reproduce the **ER2 spatial transcriptomics (ST) deconvolution** results (sample **CID4535**) using our Docker image.
...
- `INPUT_DIR`: where you placed `ST_ER2/input`
...
docker run --rm -it \
  ...
  st_deconv \
  --sc /inputs/SC_CID4535.h5ad \
  --st /inputs/ST_CID4535.h5ad \
...
- `-v "${INPUT_DIR}":/inputs:ro`  
  ...
  **Container side (`/inputs`):**
  - The pipeline will read:
    - `/inputs/SC_CID4535.h5ad`
    - `/inputs/ST_CID4535.h5ad`
...
- `--out_dir <path>`  
  ...
  **Important for reproduction:**  
  ...
  - `/outputs/.../st_model/` (copied from `ST_ER2/st_model/`)
