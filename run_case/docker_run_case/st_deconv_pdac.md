# Reproducing Results: ST Deconvolution on PDAC (pdac)

This document describes how to reproduce the **PDAC spatial transcriptomics (ST) deconvolution** results using our Docker image.

---

## 1) Download the `ST_PDAC` package

Download the folder **`ST_PDAC/`** from **`XXXX`**.
...
- `--out_dir <path>`  
  ...
  **Important for reproduction:**  
  ...
  - `/outputs/st_model/` (copied from `ST_PDAC/st_model/`)
