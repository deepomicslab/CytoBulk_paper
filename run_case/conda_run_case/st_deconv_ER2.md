# Reproducing Results: ST Deconvolution on ER2 (CID4535) — Conda Version

This document describes how to reproduce the **ER2 spatial transcriptomics (ST) deconvolution** results (sample **CID4535**) using a **Conda environment**.
...
- `input/`  
  ...
  - single-cell reference (`sc`, `.h5ad`): `SC_CID4535.h5ad`
  - spatial transcriptomics data (`st`, `.h5ad`): `ST_CID4535.h5ad`

- `st_model/`  
  **Pretrained model checkpoints** for the ER2 (CID4535) case.
...
2. **Default Top-50 selection behavior**  
   The pipeline uses a default **Top-50 selection feature** step (`top_k=50`). When this set is recomputed, ties or near-ties may be resolved differently across environments or library versions, which can further contribute to differences in outputs.
...
- Run in reproduce mode (`reproduce=True` is already set inside `st_deconv_er2.py`).
...
- `PKG_DIR`: where you placed `ST_ER2/`
...
```bash
python st_deconv_er2.py \
  --sc_adata "${DATA_DIR}/SC_CID4535.h5ad" \
  --st_adata "${DATA_DIR}/ST_CID4535.h5ad" \
  --out_dir "${OUTPUT_DIR}"
  ```
