# Reproducing Results: ST Deconvolution on Mouse MOB (mouse_mob) — Conda Version

This document describes how to reproduce the **mouse MOB spatial transcriptomics (ST) deconvolution** results using a **Conda environment**.
...
- Run the pipeline with `reproduce=True` (already set inside `st_deconv_mouse_mob.py`).
...
```bash
INPUT_DIR="/path/to/ST_mouse_mob/input"
...
```bash
cp -r /path/to/ST_mouse_mob/st_model "${OUTPUT_DIR}/st_model"
```
...
```bash
python st_deconv_mouse_mob.py \
  --sc_adata "${INPUT_DIR}/sc_layer_mob.h5ad" \
  --st_adata "${INPUT_DIR}/st_mob.h5ad" \
  --out_dir "${OUTPUT_DIR}"
  ```
